	...
	\begin{algorithmic}[1]
		\Function{LCS}{$A[1..m], B[1..n]$}
			\If{$m = 0$ or $n = 0$}
				\State{\Return{0}}
			\ElsIf{$A[1] \neq B[1]$}
				\State{$a \gets$ \Call{LCS}{$A, B[2..n]$}}
			\Else{}
				\State{$a \gets 1$}
				\While{$a < m$ and $a < n$ and $A[a] = B[a]$}
					\State{$a \gets a + 1$}
				\EndWhile{}
				\State{$a \gets$ the maximum of $a$ and \Call{LCS}{$A, B[a..n]$}}
			\EndIf{}
			\State{$b \gets$ \Call{LCS}{$A[2..m], B$}}
		...
		\Function{LOS}{$X[1..n]$}
			...
			\Else{}
				...
				\While{$i + 1 < n$}
					...
					\State{$i \gets i + 1$}
				\EndWhile
				\State{\Return{the max of $i$ and \Call{LOS}{$X[2..n]$}}}
			\EndIf{}
		\EndFunction{}
	\end{algorithmic}
\end{algorithm}
	...
	\begin{algorithmic}[1]
		\Function{LXS}{$X[1..n]$}
			...
			\Else{}
				\State{$i \gets 2$}
				\While{$i + 1 < n$}
					\If{$2 \times X[i] \geq X[i - 1] + X[i + 1]$}
						\State{\Break{}}
					\EndIf{}
					\State{$i \gets i + 1$}
				\EndWhile
				\State{\Return{the max of $1 + i$ and \Call{LXS}{$X[2..n]$}}}
			\EndIf{}
		\EndFunction{}
	\end{algorithmic}
\end{algorithm}
